...
class RoutineSetsCommands implements ICommand {
  ...
  const RoutineSetsCommands({required IRoutineSetsContract repo}) : _repo = repo;
  Future<Either<Failure, List<RoutineSet>>> addItemSets(int itemId) async {
    return await _repo.addItemSets(itemId);
  }

  Future<Either<Failure, List<RoutineSet>>> saveAllSets(List<RoutineSet> setsToSave) async {
