...
class SessionBloc extends Bloc<SessionEvent, SessionState> {
  ...
  SessionBloc({required PracticeCommands commands}) : _commands = commands, super(const _Initial()) {
    // ------------------------------------------------
    ...
    on<_StartSession>((event, emit) async {
      ...
      either.fold((l) => emit(SessionState.error(l)), (r) => emit(SessionState.loaded(r)));
    });
    ...
    on<_StopSession>((event, emit) async {
      ...
      either.fold(
        ...
        (r) => emit(SessionState.noActiveSession(lastSession: event.session, isFullSession: event.fullSession)),
      );
    });
  }
}
