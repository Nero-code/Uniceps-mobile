                  ...
                  bottom: TabBar(
                    ...
                    tabs: [
                      ...state.groups.map((group) => Tab(
                          text: group.muscleGroupTranslations[
                ...
                floatingActionButton: BlocBuilder<ExercisesV2SelectionCubit,
                  ...
                  builder: (context, state) {
                    return FloatingActionButton(
                      ...
                      onPressed: () async {
                        context.read<ItemsEditBloc>().add(AddRoutineItemsEvent(
                            dayId: widget.dayId, items: state.selected));
                        showDialog(
                            context: context,
                            builder: (_) => BlocProvider.value(
                                  value: context.read<ItemsEditBloc>(),
                                  child: AlertDialog(
                                    content: BlocConsumer<ItemsEditBloc,
                                        ItemsEditState>(
                                      listenWhen: (previous, current) =>
                                          previous is ItemsDownloadingState &&
                                          current is ItemsEditLoadedState,
                                      ...
                                      builder: (context, state) {
                                        ...
                                        return ProgressWidget(
                                            percent:
                                                state is ItemsDownloadingState
                                                    ? state.progress
                                                    : 0);
                                      },
                                    ),
                      ...
                      .map((group) => Builder(builder: (context) {
                            return ExercisesListTab(
                              ...
                              onSelect: (exercise, isSelected) {
                                isSelected
                                    ? BlocProvider.of<
                                            ExercisesV2SelectionCubit>(context)
