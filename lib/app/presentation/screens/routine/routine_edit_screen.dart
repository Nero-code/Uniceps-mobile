...
import 'package:uniceps/app/presentation/screens/routine/routine_item_edit_tab.dart';
...
class _RoutineEditScreenState extends State<RoutineEditScreen>
  ...
  var tabController = ScrollController();
  ...
  bool menuIsOpen = false, overlayIsActive = false;
  Future<void> _toggleOverlay() async {
    overlayIsActive = !overlayIsActive;
    overlayController.toggle();
  }

  Future<void> _toggleActionsMenu() async {
    if (menuIsOpen) {
      menuIsOpen = false;
      setState(() {});
  ...
  Future<List<RoutineDay>?> _reorderDays2(
      void Function(List<RoutineDay> sortedDays) onReorder,
      List<RoutineDay> days) async {
    final res = await showDialog<List<RoutineDay>>(
      context: context,
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocProvider(
      ...
      child: Scaffold(
        ...
        //     onPressed: () => showCustomOverlay(context, screenSize, () {})),
        appBar: AppBar(
          title: const Text("Routine Edit"),
        ...
        body: Column(
          children: [
            ...
            BlocBuilder<DaysEditBloc, DaysEditState>(
              builder: (context, state) {
                print("pageview built!! ${state.runtimeType}");
                ...
                if (state is DaysEditLoadedState) {
                  return Expanded(
                    ...
                    children: state.days
                        .map((e) => RoutineItemEditTab(dayId: e.id ?? 0))
                        .toList(),
                    // children: [
                    //   for (int i = 0; i < state.days.length; i++)
                    //     RoutineItemEditTab(dayId: state.days[i].id ?? 0),
                    // ],
                    // childrenDelegate: SliverChildBuilderDelegate(
                    //   (context, index) {
                    //     print(
                    //         "builder index: $index : days index ${state.days[index].index}");
                    //     return RoutineItemEditTab(
                    //         key: ValueKey(state.days[index].index),
                    //         dayId: state.days[index].id ?? 0);
                    //   },
                    // ),
                  ));
