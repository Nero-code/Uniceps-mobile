...
//     _logger.t("setLoginSession => ${_session != null}");
//   }
//
//   Future<bool> _createSession() async {
//     _logger.t("createSession");
//     if (!await _storage.containsKey(key: 'refresh')) return false;
//
//     _logger.t("createSession => found a token");
//     _logger.t("createSession => session hasExpired(${_session?.hasExpired})");
//
//     final refreshToken = _storage.read(key: 'refresh') as String;
//     if (_session == null || _session!.hasExpired) {
//       try {
//         final res = await _client.post(
//           Uri.https(ApiRoutes.domain,ApiRoutes.refreshToken "$API$HTTP_ACCESS_TOKEN"),
...
//         return false;
//       } catch (e) {
//         _logger.e("createSession => Error!", error: e);
//         return false;
//       }
//     }
//     return true;
...
//     _logger.t("ensureTokenIsValid");
//     // if no token, then use is not logged in
//     if (!await _storage.containsKey(key: 'refresh')) return false;
//     _logger.t("ensureTokenIsValid => found a token");
//     final refreshToken = _storage.read(key: 'refresh') as String;
