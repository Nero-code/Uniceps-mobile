...
class AccountRemoteSource implements IAccountRemoteSource {
  ...
  Future<MembershipModel> getUserMembership() async {
    return await _clientHelper.getHandler(
        API_V2, HTTP_MEMBERSHIP, MembershipModel.fromJson);
  }

  @override
  Future<PlanModel> getPlans() async {
    return (await _clientHelper.getListHandler(
            API_V2, HTTP_PLAN, PlanModel.fromJson,
            needsHeader: false))
        .first;
  }
  ...
  Future<PaymentResponse> buyPlan(PlanItemModel item) async {
    final res = await _clientHelper.postHandler(
        API_V2, HTTP_BUY_PLAN, {"planItemId": item.id},
